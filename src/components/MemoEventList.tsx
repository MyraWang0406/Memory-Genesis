...
export function MemoEventList({ startDate, endDate, label }: Props) {
    ...
    <section style={{ background: 'var(--card-bg)', borderRadius: 12, padding: '1rem 1.25rem', boxShadow: '0 1px 3px rgba(0,0,0,0.06)' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '0.75rem' }}>
        <h3 style={{ margin: 0, fontSize: '0.9rem', fontWeight: 700, color: 'var(--text)' }}>
          📝 {label ?? '事件备忘'}
          {events.length > 0 && <span style={{ marginLeft: 6, fontSize: '0.75rem', color: 'var(--text-muted)', fontWeight: 400 }}>{events.length} 条</span>}
        </h3>
      ...
      {events.length === 0 && !showWorkspace && (
        <div style={{ fontSize: '0.83rem', color: 'var(--text-muted)', padding: '0.5rem 0' }}>暂无事件记录</div>
      )}
